				...
				if((opponent->getCurrentHealth() > 0) && (player1->getCurrentHealth() > 0) && (combat_menu_state < 3)){
					...
					if(combat_action){
						...
						if(opponent->speedCheck() > player1->speedCheck()){
							...
							if(reflect_flag == 1){
								opponent->defend(opponent->attack(combatround));
								reflect_flag = 0;
								notification_message = "Enemy attack reflected!";
								combat_menu_state = 2;
							}else{
...
void FSDMGame::close()
{
	...
	textures.notifTextTexture->free();
...
bool FSDMGame::loadMedia()
{
	...
	textures.notifTextTexture = new LTexture;
	if(textures.notifTextTexture == NULL){
		printf("Failed to allocate notifTextTexture!\n");
		success = false;
	}	
...
// WIZARD Sprite
	...
	if(gFont == NULL){
		...
		SDL_Color textColorWhite = {255, 255, 0};
		...
		if( !textures.notifTextTexture->loadFromRenderedText("Empty", textColor)){
			printf("Falied to render notif text texture!\n");
			success = false;
		}
		...
		case 1:
			if( e.type == SDL_KEYDOWN && e.key.repeat == 0 ){
				switch(e.key.keysym.sym){
					...
					case SDLK_RETURN:
						//activate ability
						if(arrowState == 0){	//heal
							if(player1->getCurrentMana() > 30){
								
								if(player1->getDefmage() >= 17){
									if(player1->getCurrentHealth() < player1->getMaxHealth()){
										player1->healCast();
										arrowState = 0;
										combat_menu_state = 0;
									}else{
										notification_message = "Health already full.";
										combat_menu_state = 2;
										arrowState = 0;
									}
								}else{
									notification_message = "Not enough skill to cast.";
									combat_menu_state = 2;
									arrowState = 0;
								}							
							}else{
								notification_message = "Not enough mana to heal.";
								combat_menu_state = 2;
								arrowState = 0;
							}
						}else if(arrowState == 2){	//reflect
							if(reflect_flag == 0){
								if(player1->getCurrentMana() > 35){
									if(player1->getDefmage() >= 35){
										reflect_flag = 1;
										player1->reflectCast();
										arrowState = 0;
										combat_menu_state = 0;
									...
									notification_message = "Not enough mana to reflect.";
									combat_menu_state = 2;
									arrowState = 0;
								}
							}else{
								notification_message = "Already cast this turn.";
								combat_menu_state = 2;
								arrowState = 0;
							}
						}
...
void FSDMGame::updateNotificationText(string message){
	...
	if( !textures.notifTextTexture->loadFromRenderedText(message, textColor)){
		printf("Falied to render notification text texture!\n");
		success = false;
	}
}
